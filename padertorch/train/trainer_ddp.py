...
def setup_distributed(dist_backend, dist_url):
    ...
    ddp_config = DDP_Config(world_size=world_size, rank=rank, device_id=device_id)
    return ddp_config
                ...
                with self.train_timer['time_per_iteration'] as timer:
                    for minibatch_index in range(self.virtual_minibatch_size):
                        ...
                        loss, example, model_output, review = \
                            self.train_step(self.model, example, self.ddp_config.device_id)
                ...
                while True:
                    with self.validate_timer['time_per_iteration']:
                        ...
                        step_output = self.validation_step(
                            self.model, example, self.ddp_config.device_id)
                    yield step_output
