...
from padertorch.contrib.aw.transformer.transformer_blocks import RelativePositionalBiasFactory
            ...
            if not self.training or (dropout_probability > self.layer_dropout):
                if self.use_rel_pos_bias:
                    x, attn_weights, position_bias = blk(
                        x, attn_mask=src_mask, position_bias=position_bias, return_weights=self.return_weights)
                else:
...
def generate_vit_forward_mask(sz: int, grid_size: Tuple[int]) -> torch.Tensor:
    """Generates an upper-triangular matrix of -inf, with zeros on diag.
    Used for forward model, which cannot see future tokens."""
    h, w = grid_size
    assert h*w == sz
...
def generate_vit_backward_mask(sz: int, grid_size: Tuple[int]) -> torch.Tensor:
...
class TransformerViTEncoder(TransformerDecoder):
    ...
    def __init__(
            ...
            attn_dropout=0,
            layer_dropout=0,
            forward=True, # attend future
            backward=True, # attend past
            use_cls_token=False,
            rel_pos_bias_factory: Optional[RelativePositionalBiasFactory] = None,
            init_mode="default",
            return_weights=False,
    ...
    def forward(self, x, grid_size=None):
        ...
        if self.forward_attn and not self.backward_attn:
            ...
            src_mask = generate_vit_forward_mask(
            ...
            src_mask = generate_vit_backward_mask(
                seq_len, grid_size=grid_size).to(x.device)
        else:
            src_mask = None
